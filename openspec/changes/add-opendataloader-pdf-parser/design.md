...
#### Lifecycle security refinement

Artifact lifecycle code is a separate trust boundary from parse-time containment. A server-owned registry must bind an immutable document/KB owner to one canonical output root, a strict `stem_hash/run-nonce` relative path, sidecar hash, generation, and state (`active`, `deleting`, `deleted`). Registration, retry, document deletion, KB deletion, and retention use one cross-process per-owner lock and generation compare-and-swap; a run cannot have multiple active owners. A tombstone name is persisted before rename so a crash after rename is recoverable. Cache entries are invalidated by matching stored `doc_id`, not by a filesystem reference.

Deletion must not be authorized by a filename, legacy path, cache value, or `doc_status.metadata` alone. It must reject absolute/traversal paths, symlink/junction/reparse-point/hardlink entries, confirm the worker has exited, and never remove a shared parser root as a shortcut. On Linux/container, traversal and removal must use directory descriptors with no-follow semantics and an fd-safe recursive deleter. The current Windows Python runtime has neither `dir_fd` nor `O_NOFOLLOW` and reports `shutil.rmtree.avoids_symlink_attacks == False`; therefore Windows automatic artifact deletion MUST remain fail-closed until a reviewed native helper provides equivalent handle-based reparse-point-safe operations. It is not acceptable to substitute `resolve()` plus `os.replace()`/`shutil.rmtree` for that guarantee.
