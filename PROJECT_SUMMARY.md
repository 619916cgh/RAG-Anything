...
- The payload gate correctly stopped before upload because its original environment-file pattern also rejected tracked `.env.example`. It now permits only that explicit safe template while continuing to reject `.env` and `.env.local`; focused release-contract tests cover both cases.
- The first completed local staging reached the remote launcher, which stopped before any Docker action because the uploaded shell script had CRLF and Linux parsed its interpreter as `bash\\r`. The launcher now emits an LF/no-BOM copy immediately before SCP, and `.gitattributes` declares LF for all shell scripts; direct and reverse-proxy health stayed HTTP 200 after the failed attempt.
