...
def _is_incomplete_pdf_coverage_error(exc: BaseException) -> bool:
    return getattr(
        exc, "failure_code", ""
    ) == "pdf_page_coverage_incomplete" or "pdf page coverage is incomplete" in _exception_chain_text(
        exc
    )


def _is_local_odl_error(exc: BaseException) -> bool:
    """Keep explicit local OpenDataLoader failures out of remote retry paths."""
    failure_code = str(getattr(exc, "failure_code", ""))
    return (
        failure_code.startswith("odl_")
        or failure_code == "pdf_page_coverage_incomplete"
    )
            ...
            msgs = [
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "请对这张图片进行 OCR，提取所有文字内容。只输出提取的文字，不要添加任何解释。",
                        },
                        {
                            "type": "image_url",
...
async def create_rag(parser=None, working_dir=None, chunking_strategy=None):
    ...
    async def vision_func(
        ...
        **kw,
        ...
        if messages is not None:
            ...
            return await llm_func(prompt, system_prompt, history_messages or [], **kw)
    ...
    chunking_map = {
        "fixed_size": None,
        "recursive": __import__(
            "raganything.chunking", fromlist=["recursive_chunking"]
        ).recursive_chunking,
        "sentence": __import__(
            "raganything.chunking", fromlist=["sentence_chunking"]
        ).sentence_chunking,
        "structure": __import__(
            "raganything.chunking", fromlist=["structure_chunking"]
        ).structure_chunking,
        "semantic": __import__(
            "raganything.chunking", fromlist=["make_semantic_chunking"]
        ).make_semantic_chunking(_embed_wrapper),
        "agentic": __import__(
            "raganything.chunking", fromlist=["make_agentic_chunking"]
        ).make_agentic_chunking(_llm_wrapper, LLM_MODEL),
    }
    ...
    lightrag_kwargs = {
        ...
        "embedding_func_max_async": _env_int(
            "ENTITY_EXTRACT_CONCURRENCY", 3, max_val=16
        ),
    ...
    config = RAGAnythingConfig(
        ...
        enable_image_processing=os.getenv("ENABLE_IMAGE_PROCESSING", "true").lower()
        == "true",
        ...
        entity_extraction_min_degree=int(
            os.getenv("ENTITY_EXTRACTION_MIN_DEGREE", "0")
        ),
    )
...
async def _await_pending_background_tasks() -> None:
    ...
    try:
        done, still_pending = await asyncio.wait(pending, timeout=_BG_TASK_MAX_WAIT)
        if still_pending:
            print(
...
async def process_file(
    ...
    print(
        f"[WORKER] 开始处理: file={filename} kb={kb_name} dir={target_dir} strategy={strategy_name}",
        flush=True,
    )
    ...
    try:
        ...
        print(
            f"[PROGRESS] phase=model-preflight status=start file={filename}", flush=True
        )
        await _preflight_embedding_service(rag)
        print(
            f"[PROGRESS] phase=model-preflight status=done file={filename}", flush=True
        )
        ...
        opendataloader_pdf = (
            ext == "pdf"
            and str(getattr(rag.config, "pdf_parser", "")).strip().lower()
            == "opendataloader"
        )
        if ext in PLAIN_TEXT_EXTS:
            ...
            try:
                ...
                if opendataloader_pdf:
                    # OpenDataLoader is an explicit, fail-closed PDF backend.
                    # Never substitute VLM OCR, MinerU, or another parser.
                    current_stage = "parsing"
                    raise
                if _is_retryable_external_error(e):
                ...
                if "Separator is not found" in err_msg or "chunk exceed" in err_msg:
                    print(
                        "[WORKER] 检测到大段连续文本，尝试预处理后重试...", flush=True
                    )
                    try:
                        ...
                        if md_files:
                            for mf in md_files[:3]:
                                with open(
                                    mf, "r", encoding="utf-8", errors="replace"
                                ) as f:
                                    text = f.read()
                                # 插入段落分隔符
                                text = text.replace("。", "。\n\n").replace(
                            ...
                            print(
                                "[WORKER] 未找到解析输出文件，使用 VLM OCR 兜底",
                                flush=True,
                            )
                    except Exception as e2:
                        print(
                            f"[WORKER] 预处理失败: {e2}，使用 VLM OCR 兜底", flush=True
                        )
            ...
            if ext in (
                ...
                if opendataloader_pdf and (
                    not docling_ok or (chunks_status_known and not chunks_ok)
                ):
                    raise RuntimeError(
                        "OpenDataLoader produced no durable chunks; fallback is disabled"
                    )
                needs_fallback = not docling_ok or (
                    chunks_status_known and not chunks_ok
                )
                if needs_fallback:
                    ...
                    try:
                        ...
                        if fallback_content:
                            ...
                            print(
                                f"[WORKER] VLM OCR 完成: {len(ocr_text)} 字符",
                                flush=True,
                            )
                    except Exception as e2:
        ...
        if sp.exists():
            ...
            for did, info in ds.items():
                ...
                if (
                    stored == filename
                    or stored.endswith(filename)
                    or os.path.basename(stored) == filename
                ):
                    found = True
            ...
            if not found:
                print(
                    f"[WORKER] WARNING: 文档记录未在 doc_status 中找到: {filename}",
                    flush=True,
                )
        ...
        retryable = (
            ...
            and not _is_local_odl_error(e)
            and (
                isinstance(e, RetryableExternalServiceError)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enable-image", dest="enable_image", default=None, choices=["true", "false"]
    )
    parser.add_argument(
        "--enable-table", dest="enable_table", default=None, choices=["true", "false"]
    )
    parser.add_argument(
        "--enable-equation",
        dest="enable_equation",
        default=None,
        choices=["true", "false"],
    )
    parser.add_argument(
        "--enable-video", dest="enable_video", default=None, choices=["true", "false"]
    )
    args = parser.parse_args()

    try:
        worker_exit_code = asyncio.run(
            process_file(
                args.file,
                args.kb,
                args.strategy,
                ...
                enable_equation=(
                    args.enable_equation == "true" if args.enable_equation else None
                ),
