...
- 当用户请求完成当前对话的本地改动时，智能体应完成与改动风险匹配的验证，仅暂存和提交当前任务拥有的文件。提交前必须检查 `git status`；不得夹带其他对话的未提交改动，尤其不得自行纳入 `PROJECT_SUMMARY.md`、本地部署配置、密钥、密码、令牌或 `.env` 文件。
- 提交结果必须单独报告已运行的测试、构建或静态检查，并明确区分它们与部署健康检查、流程/Worker、浏览器、迁移及生产验收的证据边界。
- 仅当用户明确说出“快速发布”或“部署到云端”时，才可以执行生产发布。发布只能通过根目录的 `deploy.ps1 -Commit <40位完整SHA>` 执行；它从完整已提交版本打包，禁止逐文件同步、从工作区打包或使用密码交互登录。
- 快速发布门禁如果因依赖、Docker/Compose、迁移、模型清单、前端锁文件/运行时输入或发布工具变更而拒绝候选提交时，必须停止并说明需要受控的基础镜像或迁移发布；不得绕过门禁、修改密钥/本地配置、清理 Docker 数据、删除卷或替换其他服务。
- 快速发布完成后，使用已配置的 SSH 密钥执行独立只读验收：核对候选镜像标签、应用健康、重启次数、直连与 Nginx 健康端点。发布失败时依赖现有远端脚本的自动回滚，不得自行清理镜像、卷或数据。
