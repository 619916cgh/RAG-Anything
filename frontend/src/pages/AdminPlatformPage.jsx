...
function StaticValue({ value }) {
  const display = Array.isArray(value)
    ...
    : typeof value === 'boolean'
      ? (value ? '开启' : '关闭')
      : (value === undefined || value === null || value === '' ? '未设置' : String(value))
  return <output className="mt-2 block min-h-10 rounded-lg border border-cloud-200 bg-cloud-100 px-3 py-2 text-sm text-ink-body">{display}</output>
}
...
export default function AdminPlatformPage({ onToast }) {
  ...
  const readOnly = deploymentReadOnly || !canWrite
  ...
  return <div className="mx-auto w-full max-w-5xl space-y-5">
    <header className="page-header page-header-divider">
      <div className="flex gap-3">
        ...
        <div><h1 className="page-title">平台管理</h1><p className="page-subtitle">{readOnly ? '查看可选模型、默认值和资源上限。' : '维护可选模型、默认值和资源上限。'}部署连接、Host、Key 与环境变量不属于此页面。</p></div>
      </div>
    ...
    {catalogError && <div role="alert" className="flex gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800"><AlertCircle size={16} className="shrink-0" />{catalogError}；已保留当前策略值。</div>}
    {deploymentReadOnly && <div role="status" className="flex gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800"><AlertCircle size={16} className="shrink-0" />此部署当前为只读状态；以下内容仅供审阅。</div>}
    ...
    <section className="card p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {[['llm_profile_id', '默认文本模型', 'llm'], ['vlm_profile_id', '默认图片理解模型', 'vlm']].map(([field, label, kind]) => <label className="block text-sm font-medium" key={field}>{label}
          {readOnly ? <StaticValue value={draft.defaults.models?.[field]} /> : <select className="select-field mt-2" disabled={Boolean(catalogError)} value={draft.defaults.models?.[field] || ''} onChange={event => setDraftPath('defaults', 'models', { ...(draft.defaults.models || {}), [field]: event.target.value })}>
    ...
    {Object.entries(DEFAULT_FIELDS).map(([section, fields]) => <section className="card p-5 sm:p-6" key={section}>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {fields.map(([field, label, type]) => <label className="block text-sm font-medium" key={field}>{label}
          {readOnly
            ? <StaticValue value={draft.defaults?.[section]?.[field]} />
            : type === 'boolean'
              ? <select className="select-field mt-2" value={String(draft.defaults?.[section]?.[field] ?? false)} onChange={event => setDefaultField(section, field, event.target.value === 'true')}><option value="true">开启</option><option value="false">关闭</option></select>
              : <input className="input-field mt-2" type={type === 'number' ? 'number' : 'text'} value={type === 'list' ? listText(draft.defaults?.[section]?.[field]) : (draft.defaults?.[section]?.[field] ?? '')} onChange={event => setDefaultField(section, field, type === 'list' ? event.target.value.split(',').map(value => value.trim()).filter(Boolean) : type === 'number' ? Number(event.target.value) : event.target.value)} />}
        </label>)}
      </div>
    </section>)}
    ...
    <section className="card p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {ALLOW_LISTS.map(([field, label]) => <label className="block text-sm font-medium" key={field}>{label}
          {readOnly ? <StaticValue value={draft.allowed?.[field]} /> : <input className="input-field mt-2" value={listText(draft.allowed?.[field])} onChange={event => setDraftPath('allowed', field, event.target.value.split(',').map(value => value.trim()).filter(Boolean))} />}
        </label>)}
      </div>
    </section>
    ...
    <section className="card p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {LIMITS.map(([field, label, min]) => <label className="block text-sm font-medium" key={field}>{label}
          {readOnly ? <StaticValue value={draft.limits?.[field]} /> : <input className="input-field mt-2" type="number" min={min} value={draft.limits?.[field] ?? ''} onChange={event => setDraftPath('limits', field, event.target.value === '' ? undefined : Number(event.target.value))} />}
        </label>)}
      </div>
    </section>
    ...
    <section className="card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
      ...
      {!readOnly && <button className="btn-primary" onClick={save} disabled={!data || saving}>{saving ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />}{saving ? '保存中…' : '保存平台策略'}</button>}
    </section>
  </div>
}
